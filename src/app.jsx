  ...
  <div style={S.card}>
    <div style={{ ...S.hdrCard(), padding: "14px 16px" }}>
      ...
      <div style={{ fontSize: 18, fontWeight: 700, color: "#F4EFE6" }}>
        Requests & Shared Trips
      </div>
    </div>

    <div style={{ padding: 14 }}>
      {(data.friends || []).filter((f) => f.status === "pending").map((f) => (
        <div key={f.id} style={{ padding: 12, borderBottom: `1px solid ${P.border}` }}>
          <div style={{ fontWeight: 700 }}>{f.name}</div>
          <div style={{ fontSize: 12, color: P.muted }}>Wants to see your camp trips</div>
          <button onClick={() => approveFriend(f.id)}>✓ Approve</button>
        </div>
      ))}
      ...
      <input
        ...
        onChange={async (e) => {
          const found = await searchProfiles(e.target.value);
  ...
  <div style={S.card}>
    ...
    <div style={{ padding: 14 }}>
      {(feedEntries || []).length === 0 ? (
        ...
        feedEntries.map((trip) => (
          <div key={trip.supabase_id || trip.id} style={{ ...S.card, marginBottom: 14, overflow: "hidden" }}>
            {trip.photos?.[0] && (
              <img
